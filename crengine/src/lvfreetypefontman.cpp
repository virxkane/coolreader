	...
	if (!item->getDef()->getName().empty()) {
		...
		for ( ;; index++ ) {
			...
			if ( face->face_flags & FT_FACE_FLAG_FIXED_WIDTH )
				fontFamily = css_ff_monospace;
			lString8 familyName(!facename.empty() ? facename : LVFreeTypeFace::familyName(face));
...
LVFontRef LVFreeTypeFontManager::GetFont(int size, int weight, bool italic, css_font_family_t family, lString8 typeface, int documentId)
{
	FONT_MAN_GUARD
#if (DEBUG_FONT_MAN==1)
	if ( _log ) {
...
bool LVFreeTypeFontManager::RegisterDocumentFont(int documentId, LVContainerRef container, lString16 name, lString8 faceName, bool bold, bool italic) {
	FONT_MAN_GUARD
...
bool LVFreeTypeFontManager::RegisterExternalFont(lString16 name, lString8 family_name, bool bold, bool italic) {
	...
	for ( ;; index++ ) {
		...
		if ( face->face_flags & FT_FACE_FLAG_FIXED_WIDTH )
			fontFamily = css_ff_monospace;
		lString8 familyName( LVFreeTypeFace::familyName(face) );
		if ( familyName=="Times" || familyName=="Times New Roman" )
			fontFamily = css_ff_serif;
